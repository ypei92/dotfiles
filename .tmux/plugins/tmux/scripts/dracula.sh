...
main()
{
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ $plugin = "time" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-time-colors" "light_purple white")
    ...
    if [ $plugin = "user" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-user-colors" "gray white")

      script="#[bold]$(whoami)@#h"
    fi

    if [ $plugin = "uptime" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-uptime-colors" "red white")

      script="$(uptime | cut -d ' ' -f 4,5 | cut -d ',' -f 1)"
    fi
  ...
  if $show_powerline; then
    ...
    tmux set-window-option -g window-status-current-format "#[fg=${white},bg=colour202] #I: #W${current_flags} "
  fi

  tmux set-window-option -g window-status-format "#[fg=${white}]#[bg=${level0}] #F#I: #W${flags} "
